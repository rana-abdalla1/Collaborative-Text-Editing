...
import CRDTNode from './CRDTNode';
import {v4 as UUID} from 'uuid';

class CRDTStructure {
    constructor() {
        this.nodes = [];
        const beginningNode = new CRDTNode('\0', UUID(), -1000.0);
        const endingNode = new CRDTNode('\0', UUID(), 1000.0);
    ...
    insertBetween(node1, node2, letter, italic, bold) {
        ...
        const newNode = new CRDTNode(letter, uniqueIdentifier, newPosition, false, italic, bold);
        return newNode;
    }
    addNode(node) {
        for (let i=0; i<this.nodes.length; i++) {
    ...
    printNodes() {
        for (let i=0; i<this.nodes.length; i++) {
            console.log(this.nodes[i].letter+' '+this.nodes[i].position+' '+this.nodes[i].tombstone);
        }
    }
    ...
    myFromArray(array) {
        ...
        for (const node of array) {
            const newNode = new CRDTNode(node.letter, node.uniqueIdentifier, node.position, node.tombstone,
                node.italic, node.bold);
