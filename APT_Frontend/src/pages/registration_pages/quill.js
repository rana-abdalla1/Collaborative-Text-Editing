...
function insertQuillNode(editor,nnode=null) {
    ...
    for (let i=1; i<nodes.length-1; i++) {
        ...
        if(nnode &&(nodes[i].uniqueIdentifier===nnode.uniqueIdentifier)&&(index<selection.index))
        {
            if(nnode.tombstone)
            {
                selection.index--;
...
function TextEditor({documentId, userid, role}) {
    ...
    // eslint-disable-next-line no-unused-vars
    useEffect(() => {
        const socket = io('http://25.18.96.118:3001/', { //dont hard code
            query: {
                userId: userid,
        ...
        editor.on('text-change', (delta, oldDelta, source) => {
            ...
            delta.ops.forEach((op) => {
                ...
                if (op.insert) {
                    ...
                    const node=crdt.insertAtIndex(op.insert, index>=2?index-1:index, isItalic, isBold);
                    crdt.addNode(node);
